...
> **关于 Claude Design 导出格式：** 每个文件是零依赖的 JSX，在浏览器中通过 Babel Standalone 实时编译。一个文件通常包含多个页面级组件。drift-cli 原生支持这种格式。
...
映射关系将设计稿组件与对应的代码文件关联起来。没有映射，drift-cli 就无法生成同步 Prompt。
...
当设计稿和代码**自上次基线以来都发生了修改**，drift-cli 就会检测到冲突，无法自动决定哪一侧优先。
...
这些文件出现在状态报告中，但 drift-cli 不会主动同步它们，除非你显式建立映射。
...
**选项 2 — 保持未映射（drift-cli 将其标记为 `new-code`，不做任何操作）：**
...
**Q：drift-cli 会直接修改我的代码文件吗？**  
A：不会。drift-cli 只生成 Markdown 格式的 Prompt 文件。所有代码修改都由你将 Prompt 粘贴给 Claude Code 或 Claude Design 后，由 AI 完成。drift-cli 是一个「Prompt 工厂」，不会触碰你的源代码。
...
**Q：设计稿导出了 HTML 文件怎么办？**  
A：drift-cli 会扫描 HTML 文件中的 `<script type="text/babel">` 块，从中提取 JSX 组件。在 `design.include` 里加上 `**/*.html` 即可启用此功能。
...
**Q：如何更新 drift-cli？**  
A：`npm update -g drift-cli` 或 `pnpm update -g drift-cli`。

**Q：Claude Design 覆盖导出了所有文件，会不会所有组件都变成 design-ahead？**  
A：不会。drift-cli 先比对文件级 hash 做快速过滤，只对文件内容确实发生变化的重新提取组件 hash。即使 Claude Design 重写了 20 个 JSX 文件，`drift diff` 也只会报告内容真正变化的组件，不会产生误报。

**Q：能在 CI 中使用 drift-cli 吗？**  
...
A：drift-cli 会自动降级为无 AI 模式。你会看到提示：`未设置 ANTHROPIC_API_KEY 或 API 调用失败，跳过 AI 分析（使用通用指引）`。其余功能完全正常。
