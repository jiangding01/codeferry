...
### 新增
- **`drift log`** 命令：查看同步队列历史，支持 `--component`、`--last <n>`、`--status` 过滤
- 同步队列状态分组展示（CONFLICT → IN-PROGRESS → PENDING → DONE → SKIPPED）
- `drift log` 输出统计栏：总计 / done / in-progress / pending / skipped / conflict 计数
- 分页提示：`--last` 截断时显示"仅显示最新 N 条，共 M 条记录"

### 修复
- `drift log` 分组标题使用 ANSI-safe `toUpperCase`（避免 chalk 转义序列中 `m` 变 `M` 破坏终端颜色）
- `drift log --last` 分页计数使用 `filteredTotal`（筛选后的数量），而非全量队列长度
...
### 新增
- **`drift sync`** 命令：生成双向同步 Prompt（`--to code` / `--to design`），支持剪贴板输出（`--copy`）或文件输出（`--out <dir>`）
- **`drift diff` AI 分析集成**：自动调用 Claude API 进行语义分析，结果展示在 diff 输出中
...
- **同步队列管理**：`drift diff` 写入 pending 队列，`drift sync` 更新为 in-progress
- **`drift snapshot --after-sync`**：仅更新队列中 in-progress 的组件，自动标记为 done

### 修复
- `drift diff` 的 `--no-ai` flag 处理：commander 将 `--no-ai` 转换为 `opts.ai = false`，正确映射为 `noAi: !opts.ai`
- `drift sync` 的内容方向路由 bug：`code-to-design` 方向下代码内容/设计内容位置互换
- `drift snapshot --after-sync` 误标记历史同步组件：改为在快照前提取 `inProgressIds`，仅标记本次进行中的组件
...
### 新增
- **`drift map`** 命令：映射关系管理（`list` / `auto` / `set` / `unset`）
...
- **`drift status`** 命令：同步状态总览，支持 `--filter` 按状态过滤
- **`drift diff --no-ai`**：纯结构 diff（不调用 AI），组件内容预览
...
- **`drift snapshot`** 命令：baseline 更新，支持 `--component` 单组件模式
